...
import type { SupportResponse, FaqTopicsResponse, FaqsResponse } from '@/types/pocketbase';
import {
 ...
 * ```
 */
export async function getSupportResourcesGroupedByCategory(): Promise<Record<string, SupportResponse[]>> {
  try {
    const response = await apiFetchSupportResources({
      perPage: 100,
      sort: 'category,order',
 ...
 * ```
 */
export async function searchFaqItems(
  keyword: string
): Promise<FaqsResponse[]> {
  try {
