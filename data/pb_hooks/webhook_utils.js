...
async function sendWebhook(destination, headersObject, payload) {
    ...
    try {
        ...
        if (response.status >= 200 && response.status < 300) {
            console.log(`Webhook to ${destination} succeeded with status ${response.status}`);
        } else {
            // Log failure with status and potentially response body
            console.error(`Webhook to ${destination} failed with status ${response.status}: ${response.body}`);
        }
    } catch (error) {
...
async function triggerWebhooks(action, collectionName, record) {
    ...
    console.log(`Webhook trigger called for ${action} on collection ${collectionName}`);

    try {
        console.log(`About to query 'webhooks' collection for ${collectionName} ${action}...`);
        console.log(`Using filter: collection = ${collectionName} && active = true && (event_type = "${eventType}" || event_type = "" || event_type = null)`);
        ...
        console.log(`Query completed. Found ${webhooks.length} potential webhooks for ${collectionName} ${action}`);
        ...
        console.log(`Payload prepared (${payload.length} chars). Sending webhooks...`);

        for (const webhook of webhooks) {
            const destination = webhook.getString('destination');
            const headersObject = webhook.unmarshalJSONField('headers');

            console.log(`Processing webhook config: ${webhook.getString('name')} -> ${destination}`);
