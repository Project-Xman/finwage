/// <reference path="../pb_data/types.d.ts" />
migrate(
  (app) => {
    const snapshot = [
      {
        authAlert: {
          emailTemplate: {
            body: "<p>Hello,</p>\n<p>We noticed a login to your {APP_NAME} account from a new location.</p>\n<p>If this was you, you may disregard this email.</p>\n<p><strong>If this wasn't you, you should immediately change your {APP_NAME} account password to revoke access from all other locations.</strong></p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>",
            subject: "Login from a new location",
          },
          enabled: true,
        },
        authRule: "",
        authToken: {
          duration: 1209600,
        },
        confirmEmailChangeTemplate: {
          body: '<p>Hello,</p>\n<p>Click on the button below to confirm your new email address.</p>\n<p>\n  <a class="btn" href="{APP_URL}/_/#/auth/confirm-email-change/{TOKEN}" target="_blank" rel="noopener">Confirm new email</a>\n</p>\n<p><i>If you didn\'t ask to change your email address, you can ignore this email.</i></p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>',
          subject: "Confirm your {APP_NAME} new email address",
        },
        createRule: "",
        deleteRule: "id = @request.auth.id",
        emailChangeToken: {
          duration: 1800,
        },
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            cost: 10,
            hidden: true,
            id: "password901924565",
            max: 0,
            min: 8,
            name: "password",
            pattern: "",
            presentable: false,
            required: true,
            system: true,
            type: "password",
          },
          {
            autogeneratePattern: "[a-zA-Z0-9_]{50}",
            hidden: true,
            id: "text2504183744",
            max: 60,
            min: 30,
            name: "tokenKey",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            exceptDomains: null,
            hidden: false,
            id: "email3885137012",
            name: "email",
            onlyDomains: null,
            presentable: false,
            required: false,
            system: true,
            type: "email",
          },
          {
            hidden: false,
            id: "bool1547992806",
            name: "emailVisibility",
            presentable: false,
            required: false,
            system: true,
            type: "bool",
          },
          {
            hidden: false,
            id: "bool256245529",
            name: "verified",
            presentable: false,
            required: false,
            system: true,
            type: "bool",
          },
          {
            autogeneratePattern: "users[0-9]{6}",
            hidden: false,
            id: "text4166911607",
            max: 150,
            min: 3,
            name: "username",
            pattern: "^[\\w][\\w\\.\\-]*$",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "users_name",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "users_avatar",
            maxSelect: 1,
            maxSize: 5242880,
            mimeTypes: [
              "image/jpeg",
              "image/png",
              "image/svg+xml",
              "image/gif",
              "image/webp",
            ],
            name: "avatar",
            presentable: false,
            protected: false,
            required: false,
            system: false,
            thumbs: null,
            type: "file",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        fileToken: {
          duration: 120,
        },
        id: "_pb_users_auth_",
        indexes: [
          "CREATE UNIQUE INDEX `__pb_users_auth__username_idx` ON `users` (username COLLATE NOCASE)",
          "CREATE UNIQUE INDEX `__pb_users_auth__email_idx` ON `users` (`email`) WHERE `email` != ''",
          "CREATE UNIQUE INDEX `__pb_users_auth__tokenKey_idx` ON `users` (`tokenKey`)",
        ],
        listRule: "id = @request.auth.id",
        manageRule: null,
        mfa: {
          duration: 1800,
          enabled: false,
          rule: "",
        },
        name: "users",
        oauth2: {
          enabled: false,
          mappedFields: {
            avatarURL: "",
            id: "",
            name: "",
            username: "username",
          },
        },
        otp: {
          duration: 180,
          emailTemplate: {
            body: "<p>Hello,</p>\n<p>Your one-time password is: <strong>{OTP}</strong></p>\n<p><i>If you didn't ask for the one-time password, you can ignore this email.</i></p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>",
            subject: "OTP for {APP_NAME}",
          },
          enabled: false,
          length: 8,
        },
        passwordAuth: {
          enabled: true,
          identityFields: ["email", "username"],
        },
        passwordResetToken: {
          duration: 1800,
        },
        resetPasswordTemplate: {
          body: '<p>Hello,</p>\n<p>Click on the button below to reset your password.</p>\n<p>\n  <a class="btn" href="{APP_URL}/_/#/auth/confirm-password-reset/{TOKEN}" target="_blank" rel="noopener">Reset password</a>\n</p>\n<p><i>If you didn\'t ask to reset your password, you can ignore this email.</i></p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>',
          subject: "Reset your {APP_NAME} password",
        },
        system: false,
        type: "auth",
        updateRule: "id = @request.auth.id",
        verificationTemplate: {
          body: '<p>Hello,</p>\n<p>Thank you for joining us at {APP_NAME}.</p>\n<p>Click on the button below to verify your email address.</p>\n<p>\n  <a class="btn" href="{APP_URL}/_/#/auth/confirm-verification/{TOKEN}" target="_blank" rel="noopener">Verify</a>\n</p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>',
          subject: "Verify your {APP_NAME} email",
        },
        verificationToken: {
          duration: 604800,
        },
        viewRule: "id = @request.auth.id",
      },
      {
        createRule: null,
        deleteRule: null,
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "byvnxyye",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "kfv3t9ry",
            max: 0,
            min: 0,
            name: "collection",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            exceptDomains: null,
            hidden: false,
            id: "ecw6uxwe",
            name: "destination",
            onlyDomains: null,
            presentable: false,
            required: true,
            system: false,
            type: "url",
          },
          {
            hidden: false,
            id: "qs3td1p2",
            maxSize: 0,
            name: "headers",
            presentable: false,
            required: false,
            system: false,
            type: "json",
          },
          {
            hidden: false,
            id: "rclfgena",
            name: "active",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "select2467634050",
            maxSelect: 2,
            name: "event_type",
            presentable: false,
            required: true,
            system: false,
            type: "select",
            values: ["create", "update", "delete"],
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "ff4ld3uohrf5e3t",
        indexes: [],
        listRule: null,
        name: "webhooks",
        system: true,
        type: "base",
        updateRule: null,
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "xkbnn98q",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "wrzebxvu",
            max: 0,
            min: 0,
            name: "slug",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            exceptDomains: [],
            hidden: false,
            id: "ph7ri5lj",
            name: "email",
            onlyDomains: [],
            presentable: false,
            required: true,
            system: false,
            type: "email",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "em5jzuq2",
            max: 0,
            min: 0,
            name: "bio",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "sgaj7vks",
            maxSelect: 1,
            maxSize: 5242880,
            mimeTypes: [],
            name: "avatar",
            presentable: false,
            protected: false,
            required: false,
            system: false,
            thumbs: [],
            type: "file",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "sdnb1vdj",
            max: 0,
            min: 0,
            name: "role",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "4rs1ru6h",
            maxSize: 2000000,
            name: "social_link",
            presentable: false,
            required: false,
            system: false,
            type: "json",
          },
          {
            hidden: false,
            id: "6oe9zckt",
            name: "active",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "tjotz8gqxaxn6xt",
        indexes: [
          "CREATE UNIQUE INDEX `idx_Mi9Uoyk` ON `authors` (\n  `slug`,\n  `email`\n)",
        ],
        listRule: "@request.auth.id != ''",
        name: "authors",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "otwzondt",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "1owdfny6",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            exceptDomains: [],
            hidden: false,
            id: "h0sjarsc",
            name: "field",
            onlyDomains: [],
            presentable: false,
            required: true,
            system: false,
            type: "url",
          },
          {
            hidden: false,
            id: "7rtelybo",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            cascadeDelete: false,
            collectionId: "4yynrrycy21emob",
            hidden: false,
            id: "yr1lundf",
            maxSelect: 1,
            minSelect: 0,
            name: "category",
            presentable: false,
            required: false,
            system: false,
            type: "relation",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "z1gvdne492j0o6t",
        indexes: [],
        listRule: "",
        name: "support",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: "",
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "bn177x9m",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "eye2xkq0",
            max: 0,
            min: 0,
            name: "slug",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "7x9nddpu",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "jy6mnmnr",
            max: 0,
            min: 0,
            name: "color",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "pytzai91",
            max: 0,
            min: 0,
            name: "icon",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ce9zfyp9",
            max: 0,
            min: 0,
            name: "count",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "4yynrrycy21emob",
        indexes: [],
        listRule: null,
        name: "category",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "vv1cntdz",
            max: 0,
            min: 0,
            name: "metric",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "oysoyi3p",
            max: 0,
            min: 0,
            name: "value",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "9wv7potk",
            max: 0,
            min: 0,
            name: "label",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "nxo6zedb",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "m1wwkuzr",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "ty7114ys4i56a6k",
        indexes: [],
        listRule: null,
        name: "status",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ggdrum8v",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "s1x67ndx",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "gujnvq829xvd95h",
        indexes: [],
        listRule: null,
        name: "security_features",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "uglce2pt",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "6ctjahoq",
            max: 0,
            min: 0,
            name: "slug",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            convertURLs: false,
            hidden: false,
            id: "sdv0hkst",
            maxSize: 0,
            name: "content",
            presentable: false,
            required: false,
            system: false,
            type: "editor",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "n1vlzpo7",
            max: 0,
            min: 0,
            name: "excerpt",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            cascadeDelete: false,
            collectionId: "tjotz8gqxaxn6xt",
            hidden: false,
            id: "v8w3dvcy",
            maxSelect: 1,
            minSelect: 0,
            name: "author",
            presentable: false,
            required: false,
            system: false,
            type: "relation",
          },
          {
            cascadeDelete: false,
            collectionId: "4yynrrycy21emob",
            hidden: false,
            id: "eoacgy7c",
            maxSelect: 1,
            minSelect: 0,
            name: "category",
            presentable: false,
            required: false,
            system: false,
            type: "relation",
          },
          {
            hidden: false,
            id: "kuxbnnru",
            maxSelect: 1,
            maxSize: 5242880,
            mimeTypes: ["image/jpeg", "image/png", "image/webp"],
            name: "featured_image",
            presentable: false,
            protected: false,
            required: false,
            system: false,
            thumbs: [],
            type: "file",
          },
          {
            hidden: false,
            id: "tq1ujgwr",
            maxSize: 2000000,
            name: "tags",
            presentable: false,
            required: false,
            system: false,
            type: "json",
          },
          {
            hidden: false,
            id: "xnnhjbjb",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "slmb2cig",
            name: "published",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "p7jcrfsd",
            max: "",
            min: "",
            name: "published_date",
            presentable: false,
            required: false,
            system: false,
            type: "date",
          },
          {
            hidden: false,
            id: "hibcafiu",
            max: null,
            min: 0,
            name: "views",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "t9kcwli6tj7h0fk",
        indexes: ["CREATE UNIQUE INDEX `idx_k3PbFhU` ON `blogs` (`slug`)"],
        listRule: "published = true || @request.auth.id != ''",
        name: "blogs",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: "published = true || @request.auth.id != ''",
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "qngzqpdf",
            max: 0,
            min: 0,
            name: "step",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ophix9yc",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "tqke4exb",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "n2vnppib",
            max: 0,
            min: 0,
            name: "icon",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "ygr9vw2r",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            cascadeDelete: false,
            collectionId: "4yynrrycy21emob",
            hidden: false,
            id: "2g48ilwy",
            maxSelect: 1,
            minSelect: 0,
            name: "category",
            presentable: false,
            required: false,
            system: false,
            type: "relation",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "1orzmnhcxugwtya",
        indexes: [],
        listRule: null,
        name: "process_steps",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "dlnxelkn",
            max: 0,
            min: 0,
            name: "icon",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "4cuh8dds",
            max: 0,
            min: 0,
            name: "bg_color",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "n3jqswqu",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "1hakyddj",
            maxSize: 2000000,
            name: "points",
            presentable: false,
            required: true,
            system: false,
            type: "json",
          },
          {
            hidden: false,
            id: "7l1a7fnf",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "pexw7ocvmo3ohmj",
        indexes: [],
        listRule: null,
        name: "cta_cards",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "aqlvl7be",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "vlfhw9rt",
            max: 0,
            min: 0,
            name: "slug",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "8lcaazyy",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "hvfgyco5",
            max: null,
            min: null,
            name: "price",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "izrmuzgd",
            max: 0,
            min: 0,
            name: "currency",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "s5lrasgu",
            maxSize: 2000000,
            name: "features",
            presentable: false,
            required: false,
            system: false,
            type: "json",
          },
          {
            hidden: false,
            id: "hex1ghac",
            maxSize: 2000000,
            name: "limitations",
            presentable: false,
            required: false,
            system: false,
            type: "json",
          },
          {
            hidden: false,
            id: "6a2qchii",
            name: "is_popular",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "mjok3fgx",
            name: "is_enterprise",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "ipgbwbhy",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "akrcm7pq",
            name: "active",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "x4ra2478c83gn97",
        indexes: [
          "CREATE UNIQUE INDEX `idx_nbKj3i7` ON `pricing_plans` (`slug`)",
        ],
        listRule: "active = true",
        name: "pricing_plans",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: "active = true",
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "crmjfquy",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ci7hvfsx",
            max: 0,
            min: 0,
            name: "content",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "cz6xyheh",
            max: "",
            min: "",
            name: "published_date",
            presentable: false,
            required: true,
            system: false,
            type: "date",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "vgxl0xjd",
            max: 0,
            min: 0,
            name: "source",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            exceptDomains: null,
            hidden: false,
            id: "ktfpm4eb",
            name: "url",
            onlyDomains: null,
            presentable: false,
            required: false,
            system: false,
            type: "url",
          },
          {
            hidden: false,
            id: "8g7talea",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "zbk2mvju",
            name: "published",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "k3i78yg4pmvvo4f",
        indexes: [],
        listRule: "featured = true || published = true",
        name: "press",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: "featured = true || published = true",
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            hidden: false,
            id: "meqelfbk",
            max: 2100,
            min: 1900,
            name: "year",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ob1gietb",
            max: 0,
            min: 0,
            name: "event",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "orzd0jgd",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "8kusdpsr",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "wtfuoaby",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "rgkm2bbs5fdp4cm",
        indexes: [],
        listRule: null,
        name: "company_milestones",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "edjwxxkv",
            max: 0,
            min: 0,
            name: "icon",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "bshtzezw",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "rbtwdjeq",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "axbm3qnp",
            maxSize: 2000000,
            name: "details",
            presentable: false,
            required: true,
            system: false,
            type: "json",
          },
          {
            hidden: false,
            id: "0lnmkfn7",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "cfwk41ovx3devvo",
        indexes: [],
        listRule: null,
        name: "compliance_items",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "2lbq8ben",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "uqxyslee",
            max: 0,
            min: 0,
            name: "slug",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "datzkejs",
            maxSelect: 1,
            maxSize: 5242880,
            mimeTypes: [],
            name: "logo",
            presentable: false,
            protected: false,
            required: false,
            system: false,
            thumbs: [],
            type: "file",
          },
          {
            exceptDomains: null,
            hidden: false,
            id: "mtx26qul",
            name: "website",
            onlyDomains: null,
            presentable: false,
            required: false,
            system: false,
            type: "url",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "o5otbpdt",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "fueqqzcs",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "1tv5eu5d",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "1bwx0oy1",
            name: "active",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            cascadeDelete: false,
            collectionId: "4yynrrycy21emob",
            hidden: false,
            id: "s04gujbj",
            maxSelect: 1,
            minSelect: 0,
            name: "category",
            presentable: false,
            required: false,
            system: false,
            type: "relation",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "ktodfpwd0ga82p6",
        indexes: ["CREATE INDEX `idx_6q6QPpI` ON `partners` (`slug`)"],
        listRule: null,
        name: "partners",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "1odhowks",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "k54v0loy",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "lx4no5ua",
            max: 0,
            min: 0,
            name: "icon",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "n9te6w7i",
            max: 0,
            min: 0,
            name: "type",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            exceptDomains: null,
            hidden: false,
            id: "k1sxdtbu",
            name: "action_url",
            onlyDomains: null,
            presentable: false,
            required: false,
            system: false,
            type: "url",
          },
          {
            hidden: false,
            id: "nnsqfqxr",
            name: "is_featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "84jmpelk1jgxyeo",
        indexes: [],
        listRule: null,
        name: "contact_options",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "mfkph45z",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "hqhnbunz",
            max: 0,
            min: 0,
            name: "address",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "viyarayc",
            max: 0,
            min: 0,
            name: "city",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "g0j7sovl",
            max: 0,
            min: 0,
            name: "state",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "0zeuhkcj",
            max: 0,
            min: 0,
            name: "zip",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "jwfsuijw",
            max: 0,
            min: 0,
            name: "country",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "qtboht37",
            max: 0,
            min: 0,
            name: "phone",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            exceptDomains: null,
            hidden: false,
            id: "fbniv2ge",
            name: "email",
            onlyDomains: null,
            presentable: false,
            required: false,
            system: false,
            type: "email",
          },
          {
            hidden: false,
            id: "ewypgkvg",
            maxSize: 2000000,
            name: "coordinates",
            presentable: false,
            required: false,
            system: false,
            type: "json",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "oonbg3mkpcxmou7",
        indexes: [],
        listRule: null,
        name: "locations",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "i3e899v5",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "exfbhfrd",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "kfd3q4ru",
            max: 0,
            min: 0,
            name: "icon",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            cascadeDelete: false,
            collectionId: "4yynrrycy21emob",
            hidden: false,
            id: "kssuoo7v",
            maxSelect: 1,
            minSelect: 0,
            name: "category",
            presentable: false,
            required: false,
            system: false,
            type: "relation",
          },
          {
            hidden: false,
            id: "dmbttfnj",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "hip9hcpv0kdwaxk",
        indexes: [],
        listRule: null,
        name: "employee_benefits",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "hbcnr0rr",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "udso7ljv",
            max: 0,
            min: 0,
            name: "role",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "bwwvuufo",
            max: 0,
            min: 0,
            name: "bio",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "neqrxtzw",
            maxSelect: 1,
            maxSize: 5242880,
            mimeTypes: [],
            name: "image",
            presentable: false,
            protected: false,
            required: false,
            system: false,
            thumbs: [],
            type: "file",
          },
          {
            exceptDomains: null,
            hidden: false,
            id: "u9mbdzlc",
            name: "email",
            onlyDomains: null,
            presentable: false,
            required: false,
            system: false,
            type: "email",
          },
          {
            hidden: false,
            id: "7cot6xzz",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "hzaraca2",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "qy8m5olx",
            maxSize: 2000000,
            name: "social_links",
            presentable: false,
            required: false,
            system: false,
            type: "json",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "k8090bx1q1ng2xm",
        indexes: [],
        listRule: null,
        name: "leadership",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "oa4rgpvl",
            max: 0,
            min: 0,
            name: "value",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "wzjbvttd",
            max: 0,
            min: 0,
            name: "label",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "isyn5ejy",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "7gmmuh1621i3mh4",
        indexes: [],
        listRule: null,
        name: "employer_stats",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "udod7xmi",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "4jhmaxh1",
            max: 0,
            min: 0,
            name: "department",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "4cefi8du",
            max: 0,
            min: 0,
            name: "location",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ztmro8mn",
            max: 0,
            min: 0,
            name: "type",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "qeslyazv",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "yjdecyzj",
            max: 0,
            min: 0,
            name: "requirements",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ob7cxeld",
            max: 0,
            min: 0,
            name: "salary_range",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "avkkn6py",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "za7ctpxz",
            max: 0,
            min: 0,
            name: "status",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "fmuq2cknmztobe3",
        indexes: [],
        listRule: "status = 'open'",
        name: "jobs",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: "status = 'open'",
      },
      {
        createRule: "",
        deleteRule: "",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ufujh6y7",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            exceptDomains: [],
            hidden: false,
            id: "znvvacyc",
            name: "email",
            onlyDomains: [],
            presentable: false,
            required: true,
            system: false,
            type: "email",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "c79yn5tb",
            max: 0,
            min: 0,
            name: "company",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "fvoeondv",
            max: null,
            min: null,
            name: "phone",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "kl5qreyj",
            max: 0,
            min: 0,
            name: "message",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "vd0u089p",
            maxSelect: 1,
            name: "interest",
            presentable: false,
            required: false,
            system: false,
            type: "select",
            values: ["demo", "pricing", "contact", "other"],
          },
          {
            hidden: false,
            id: "ozgmpeef",
            maxSelect: 1,
            name: "status",
            presentable: false,
            required: false,
            system: false,
            type: "select",
            values: ["new", "contacted", "converted", "closed"],
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "yc2q361kh0c3oom",
        indexes: [],
        listRule: null,
        name: "enquiries",
        system: false,
        type: "base",
        updateRule: "",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "nsx9tugk",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "imdwtjrx",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "bhaifdr2",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "g8bfvgmdclvklv9",
        indexes: [],
        listRule: null,
        name: "faq_topics",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "3izo9z49",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "nlddmwib",
            max: 0,
            min: 0,
            name: "slug",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "iszfb5sp",
            maxSelect: 1,
            maxSize: 5242880,
            mimeTypes: [],
            name: "logo",
            presentable: false,
            protected: false,
            required: false,
            system: false,
            thumbs: [],
            type: "file",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "tfoyj5kt",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            exceptDomains: null,
            hidden: false,
            id: "fma3vb82",
            name: "documentation_url",
            onlyDomains: null,
            presentable: false,
            required: false,
            system: false,
            type: "url",
          },
          {
            hidden: false,
            id: "8jao6cma",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "dqlqedeq",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "epqxte5n",
            name: "active",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            cascadeDelete: false,
            collectionId: "4yynrrycy21emob",
            hidden: false,
            id: "myuya2l3",
            maxSelect: 1,
            minSelect: 0,
            name: "category",
            presentable: false,
            required: false,
            system: false,
            type: "relation",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "fhgitrs72coe5i0",
        indexes: ["CREATE INDEX `idx_f5elfgm` ON `integrations` (`slug`)"],
        listRule: "active = true",
        name: "integrations",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: "active = true",
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "wbkrjnrc",
            max: 0,
            min: 0,
            name: "question",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "wnjybckt",
            max: 0,
            min: 0,
            name: "answer",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            cascadeDelete: false,
            collectionId: "4yynrrycy21emob",
            hidden: false,
            id: "8x4duw2e",
            maxSelect: 1,
            minSelect: 0,
            name: "category",
            presentable: false,
            required: false,
            system: false,
            type: "relation",
          },
          {
            hidden: false,
            id: "0lwv1dre",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "gosueoax",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "cgmtnwhi",
            max: 0,
            min: 0,
            name: "category_text",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "ajgjsbzl5fjq921",
        indexes: [],
        listRule: "",
        name: "faqs",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "96enl1xk",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "eyin75zn",
            max: 0,
            min: 0,
            name: "slug",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "8pt5paru",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "kxeuzwuu",
            max: 0,
            min: 0,
            name: "icon",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "lprrlcov",
            maxSelect: 1,
            maxSize: 5242880,
            mimeTypes: [],
            name: "image",
            presentable: false,
            protected: false,
            required: false,
            system: false,
            thumbs: [],
            type: "file",
          },
          {
            cascadeDelete: false,
            collectionId: "4yynrrycy21emob",
            hidden: false,
            id: "j0chg0wp",
            maxSelect: 1,
            minSelect: 0,
            name: "category",
            presentable: false,
            required: false,
            system: false,
            type: "relation",
          },
          {
            hidden: false,
            id: "scdbc60t",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "0m9o6mtt",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "cxstclca",
            name: "active",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "7mj7xmewunl692z",
        indexes: ["CREATE UNIQUE INDEX `idx_5Okfkm5` ON `features` (`slug`)"],
        listRule: "active = true",
        name: "features",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: "active = true",
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "tenw59sm",
            max: 0,
            min: 0,
            name: "name",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ps1kecba",
            max: 0,
            min: 0,
            name: "company",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "u9hx00ga",
            max: 0,
            min: 0,
            name: "position",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "ii0r4ofn",
            max: 0,
            min: 0,
            name: "quote",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "nx9dkmxi",
            max: 5,
            min: 0,
            name: "rating",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "lzzbll8f",
            maxSelect: 1,
            maxSize: 5242880,
            mimeTypes: ["image/png", "image/jpeg", "image/webp"],
            name: "image",
            presentable: false,
            protected: false,
            required: false,
            system: false,
            thumbs: [],
            type: "file",
          },
          {
            hidden: false,
            id: "igj0h1bp",
            name: "verified",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "tpgxwpgi",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "lwlsadaa",
            name: "order",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "wvhl8adve9nj1wd",
        indexes: [],
        listRule: "verified = true",
        name: "testimonials",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: "verified = true",
      },
      {
        createRule: "@request.auth.id != ''",
        deleteRule: "@request.auth.id != ''",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "pvsgsb0n",
            max: 0,
            min: 0,
            name: "title",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "qarjsui9",
            max: 0,
            min: 0,
            name: "description",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: false,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "xxibvjbb",
            max: 0,
            min: 0,
            name: "icon",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: false,
            type: "text",
          },
          {
            hidden: false,
            id: "gqi1xtgf",
            max: null,
            min: null,
            name: "order",
            onlyInt: false,
            presentable: false,
            required: false,
            system: false,
            type: "number",
          },
          {
            hidden: false,
            id: "2nrvhnsx",
            name: "featured",
            presentable: false,
            required: false,
            system: false,
            type: "bool",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: false,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: false,
            type: "autodate",
          },
        ],
        id: "ke3h2ioma82kf12",
        indexes: [],
        listRule: null,
        name: "values",
        system: false,
        type: "base",
        updateRule: "@request.auth.id != ''",
        viewRule: null,
      },
      {
        authAlert: {
          emailTemplate: {
            body: "<p>Hello,</p>\n<p>We noticed a login to your {APP_NAME} account from a new location.</p>\n<p>If this was you, you may disregard this email.</p>\n<p><strong>If this wasn't you, you should immediately change your {APP_NAME} account password to revoke access from all other locations.</strong></p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>",
            subject: "Login from a new location",
          },
          enabled: true,
        },
        authRule: "",
        authToken: {
          duration: 1209600,
        },
        confirmEmailChangeTemplate: {
          body: '<p>Hello,</p>\n<p>Click on the button below to confirm your new email address.</p>\n<p>\n  <a class="btn" href="{APP_URL}/_/#/auth/confirm-email-change/{TOKEN}" target="_blank" rel="noopener">Confirm new email</a>\n</p>\n<p><i>If you didn\'t ask to change your email address, you can ignore this email.</i></p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>',
          subject: "Confirm your {APP_NAME} new email address",
        },
        createRule: null,
        deleteRule: null,
        emailChangeToken: {
          duration: 1800,
        },
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            cost: 0,
            hidden: true,
            id: "password901924565",
            max: 0,
            min: 8,
            name: "password",
            pattern: "",
            presentable: false,
            required: true,
            system: true,
            type: "password",
          },
          {
            autogeneratePattern: "[a-zA-Z0-9]{50}",
            hidden: true,
            id: "text2504183744",
            max: 60,
            min: 30,
            name: "tokenKey",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            exceptDomains: null,
            hidden: false,
            id: "email3885137012",
            name: "email",
            onlyDomains: null,
            presentable: false,
            required: true,
            system: true,
            type: "email",
          },
          {
            hidden: false,
            id: "bool1547992806",
            name: "emailVisibility",
            presentable: false,
            required: false,
            system: true,
            type: "bool",
          },
          {
            hidden: false,
            id: "bool256245529",
            name: "verified",
            presentable: false,
            required: false,
            system: true,
            type: "bool",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: true,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: true,
            type: "autodate",
          },
        ],
        fileToken: {
          duration: 120,
        },
        id: "pbc_3142635823",
        indexes: [
          "CREATE UNIQUE INDEX `idx_tokenKey_pbc_3142635823` ON `_superusers` (`tokenKey`)",
          "CREATE UNIQUE INDEX `idx_email_pbc_3142635823` ON `_superusers` (`email`) WHERE `email` != ''",
        ],
        listRule: null,
        manageRule: null,
        mfa: {
          duration: 1800,
          enabled: false,
          rule: "",
        },
        name: "_superusers",
        oauth2: {
          enabled: false,
          mappedFields: {
            avatarURL: "",
            id: "",
            name: "",
            username: "",
          },
        },
        otp: {
          duration: 180,
          emailTemplate: {
            body: "<p>Hello,</p>\n<p>Your one-time password is: <strong>{OTP}</strong></p>\n<p><i>If you didn't ask for the one-time password, you can ignore this email.</i></p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>",
            subject: "OTP for {APP_NAME}",
          },
          enabled: false,
          length: 8,
        },
        passwordAuth: {
          enabled: true,
          identityFields: ["email"],
        },
        passwordResetToken: {
          duration: 1800,
        },
        resetPasswordTemplate: {
          body: '<p>Hello,</p>\n<p>Click on the button below to reset your password.</p>\n<p>\n  <a class="btn" href="{APP_URL}/_/#/auth/confirm-password-reset/{TOKEN}" target="_blank" rel="noopener">Reset password</a>\n</p>\n<p><i>If you didn\'t ask to reset your password, you can ignore this email.</i></p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>',
          subject: "Reset your {APP_NAME} password",
        },
        system: true,
        type: "auth",
        updateRule: null,
        verificationTemplate: {
          body: '<p>Hello,</p>\n<p>Thank you for joining us at {APP_NAME}.</p>\n<p>Click on the button below to verify your email address.</p>\n<p>\n  <a class="btn" href="{APP_URL}/_/#/auth/confirm-verification/{TOKEN}" target="_blank" rel="noopener">Verify</a>\n</p>\n<p>\n  Thanks,<br/>\n  {APP_NAME} team\n</p>',
          subject: "Verify your {APP_NAME} email",
        },
        verificationToken: {
          duration: 259200,
        },
        viewRule: null,
      },
      {
        createRule: null,
        deleteRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text455797646",
            max: 0,
            min: 0,
            name: "collectionRef",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text127846527",
            max: 0,
            min: 0,
            name: "recordRef",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text2462348188",
            max: 0,
            min: 0,
            name: "provider",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text1044722854",
            max: 0,
            min: 0,
            name: "providerId",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: true,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: true,
            type: "autodate",
          },
        ],
        id: "pbc_2281828961",
        indexes: [
          "CREATE UNIQUE INDEX `idx_externalAuths_record_provider` ON `_externalAuths` (collectionRef, recordRef, provider)",
          "CREATE UNIQUE INDEX `idx_externalAuths_collection_provider` ON `_externalAuths` (collectionRef, provider, providerId)",
        ],
        listRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
        name: "_externalAuths",
        system: true,
        type: "base",
        updateRule: null,
        viewRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
      },
      {
        createRule: null,
        deleteRule: null,
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text455797646",
            max: 0,
            min: 0,
            name: "collectionRef",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text127846527",
            max: 0,
            min: 0,
            name: "recordRef",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text1582905952",
            max: 0,
            min: 0,
            name: "method",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: true,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: true,
            type: "autodate",
          },
        ],
        id: "pbc_2279338944",
        indexes: [
          "CREATE INDEX `idx_mfas_collectionRef_recordRef` ON `_mfas` (collectionRef,recordRef)",
        ],
        listRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
        name: "_mfas",
        system: true,
        type: "base",
        updateRule: null,
        viewRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
      },
      {
        createRule: null,
        deleteRule: null,
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text455797646",
            max: 0,
            min: 0,
            name: "collectionRef",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text127846527",
            max: 0,
            min: 0,
            name: "recordRef",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            cost: 8,
            hidden: true,
            id: "password901924565",
            max: 0,
            min: 0,
            name: "password",
            pattern: "",
            presentable: false,
            required: true,
            system: true,
            type: "password",
          },
          {
            autogeneratePattern: "",
            hidden: true,
            id: "text3866985172",
            max: 0,
            min: 0,
            name: "sentTo",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: false,
            system: true,
            type: "text",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: true,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: true,
            type: "autodate",
          },
        ],
        id: "pbc_1638494021",
        indexes: [
          "CREATE INDEX `idx_otps_collectionRef_recordRef` ON `_otps` (collectionRef, recordRef)",
        ],
        listRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
        name: "_otps",
        system: true,
        type: "base",
        updateRule: null,
        viewRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
      },
      {
        createRule: null,
        deleteRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
        fields: [
          {
            autogeneratePattern: "[a-z0-9]{15}",
            hidden: false,
            id: "text3208210256",
            max: 15,
            min: 15,
            name: "id",
            pattern: "^[a-z0-9]+$",
            presentable: false,
            primaryKey: true,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text455797646",
            max: 0,
            min: 0,
            name: "collectionRef",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text127846527",
            max: 0,
            min: 0,
            name: "recordRef",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            autogeneratePattern: "",
            hidden: false,
            id: "text4228609354",
            max: 0,
            min: 0,
            name: "fingerprint",
            pattern: "",
            presentable: false,
            primaryKey: false,
            required: true,
            system: true,
            type: "text",
          },
          {
            hidden: false,
            id: "autodate2990389176",
            name: "created",
            onCreate: true,
            onUpdate: false,
            presentable: false,
            system: true,
            type: "autodate",
          },
          {
            hidden: false,
            id: "autodate3332085495",
            name: "updated",
            onCreate: true,
            onUpdate: true,
            presentable: false,
            system: true,
            type: "autodate",
          },
        ],
        id: "pbc_4275539003",
        indexes: [
          "CREATE UNIQUE INDEX `idx_authOrigins_unique_pairs` ON `_authOrigins` (collectionRef, recordRef, fingerprint)",
        ],
        listRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
        name: "_authOrigins",
        system: true,
        type: "base",
        updateRule: null,
        viewRule:
          "@request.auth.id != '' && recordRef = @request.auth.id && collectionRef = @request.auth.collectionId",
      },
    ];

    return app.importCollections(snapshot, false);
  },
  (app) => {
    return null;
  },
);
